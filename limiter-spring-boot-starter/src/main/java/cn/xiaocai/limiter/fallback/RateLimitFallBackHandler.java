...
import cn.xiaocai.limiter.annotation.*;
...
public class RateLimitFallBackHandler implements FallBackHandler{
    ...
    private static JavaType getJavaType(Type type) {
        if (type instanceof ParameterizedType) {
            ...
            return defaultInstance().constructParametricType(rowClass, javaTypes);
        } else {
            //简单类型直接用该类构建
            return defaultInstance().constructParametricType((Class)type, new JavaType[0]);
        }
    }
}
